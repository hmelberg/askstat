---
id: ess
navn: European Social Survey (ESS Data Portal API)
utgiver: ESS ERIC / Sikt
tillit: offisiell
tilgang: rest
...
auth:
  type: api_key
  env: ESS_API_KEY
  plassering: query:userId
tags: [mikro]
order: 21
---

# European Social Survey (ESS Data Portal API)

## Kort

Survey-mikrodata om holdninger/velferd/helse/tillit/livskvalitet, ~30 europeiske land inkl. Norge, 11 runder 2002–2023, personnivå. Levert per fil-DOI (én fil per runde/utgave) som parquet (default)/csv/sav/dta; ess11-csv ≈ 32 MB — foretrekk `fileFormat=parquet` for store filer/runder.
...
European Social Survey — attitudes and welfare survey microdata for around 30 European countries (incl. Norway), 2002 onward, delivered per survey round.
