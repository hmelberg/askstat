...
(function (global) {
  ...
  var LIB_REGISTRY = {
    ...
    // Ui-tilstand.
    // deps: ['ui_core'] — fase 3: ui_mpy.py importerer selv shared/ui_core.py
    // (dedup pyodide/brython/micropython), må derfor registreres FØR ui_mpy
    // selv (deps kjøres først, se ensureLibs).
    ui_mpy:             { aliases: ['ui'], deps: ['ui_core'],
    ...
    // shared/, Leaflet lastes lazy ved `import folium` (CSS statisk i
    // index.html-head).
    folium_mpy:         { aliases: ['folium'], deps: ['folium_core'],
                          js: [
    ...
    // gjenbruker plotly-shimet (deps sørger for rekkefølgen).
    lifelines_mpy:      { aliases: ['lifelines', 'lifelines.statistics'],
                          deps: ['lifelines_core', 'plotly_express_mpy',
                                 'pandas_mpy'], js: [] },
    ...
    tabulator_mpy:      { aliases: ['tabulator'], deps: ['tabulator_core'], js: [] },
  ...
  async function ensureLibs(mod, names, _visiting) {
    ...
    for (var i = 0; i < names.length; i++) {
      ...
      // shared/, ikke fra micropython/ — se ui_core-oppføringen i LIB_REGISTRY).
      var source = await fetchText(entry.path || ('micropython/' + name + '.py'));
