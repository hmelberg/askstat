...
| 2026-08-01 | — | INFRA-FUNN (dbnomics dimensions=) | Oppfølger av OECD-runden, samme feilklasse i en kilde vi allerede hadde. `translateCanonical` avviste `filters=` for dbnomics og ba modellen «snevre inn med dimensjonsfiltre i stien» — en instruks grammatikken ikke tar. API-et HAR filtrering: `?dimensions=<url-enkodet JSON med lister>` på sti-formen `/v22/series/<provider>/<dataset>`. Målt live: IMF/WEO ufiltrert = 8624 serier mot 1000-taket (dbnomicsColumns KASTER, dvs. blindvei); med `filters={"weo-country": ["NOR","SWE"]}` = 88 serier, 4488 rader, kun NOR/SWE. Fikset i fire lag: filters→dimensions i translateCanonical; countries/indicators peker nå på filters= i stedet for serie-masken; overflow-feilen likeså; og `dbnomicsMetadata` gir nå dimensjons-KODER + verdikoder (før: {lesbar etikett: antall}, som ikke kunne brukes til å bygge et filter i det hele tatt). Live smoke-test avdekket i tillegg at `weo-country` har 196 verdier — landkoden falt utenfor 40-taket, så `find=` er nå plumbet gjennom til dbnomics som for pxweb/sdmx. Låser opp de ~100 dbnomics-leverandørene som alt sto i registeret. |
