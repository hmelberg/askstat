...
**Dato:** 2026-08-05 · **Status:** utkast til godkjenning (brainstorm 2026-08-05), deretter implementasjonsplan
...
- **Språkvalg defaultes fra browser-locale** (reverserer engelsk-først-valget
  ...
  et manuelt valg.
- **Startspråk:** en (kilde) + no, da, sv, fi, is, de, fr, es, pt, zh.
...
byrånavn). V1-tabell: Norden + DE, FR, ES, PT, BR, CN, NL, IT, UK, US.
...
- **Ordbøker:** `js/i18n/<lang>.js` for no, da, sv, fi, is, de, fr, es,
  pt, zh. `SUPPORTED`-lista utvides. Omfang: ~60–80 ask-strenger + tagline/
...
- **Locale-default:** `detectInitialLang()`: lagret valg → eksakt
  ...
  sv→SE); tvetydige (de/fr/es/pt/zh/en) → internasjonal default.
...
4. **Språk:** fallbackkjede → ask-wiring (engelske nøkler) → ordbøker
   (10 språk) → locale-default + region→pakke → `ui_lang`-utvidelse +
